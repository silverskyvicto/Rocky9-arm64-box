...
build: extract-boot validate
	packer build -force -var "rocky_iso_checksum=$(ROCKY_ISO_CHECKSUM)" $(PACKER_FILE)
...
validate:
	...
	packer validate -var 'output_directory=.packer-validate-output' -var "rocky_iso_checksum=$(ROCKY_ISO_CHECKSUM)" $(PACKER_FILE)
